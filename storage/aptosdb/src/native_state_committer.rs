...
impl NativeStateCommitter {
        ...
        P: IntoIterator<Item = (StateKey, WriteOp)>,
    {
        ...
        for (state_key, op) in position_writes {
            ...
            let shard = PositionDb::shard_of_state_key(&state_key);
        ...
        // Write each shard's batch independently. Errors short-circuit —
        // sequential write order means a partial failure leaves the
        // first-N shards committed and N+1..15 untouched, but there is
        // no in-memory mirror update yet so a restart re-applies from
        // the canonical DB without inconsistency.
        for (shard, maybe_batch) in pos_batches.into_iter().enumerate() {
            if let Some(batch) = maybe_batch {
                self.position_db
                    .shard(shard)
                    .write_schemas(batch)
                    .map_err(|e| {
                        AptosDbError::Other(format!("position_db shard {shard} commit failed: {e}"))
                    })?;
            }
        }
