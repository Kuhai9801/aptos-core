...
pub enum ExecutionErrorKind {
    ...
    InvalidOperation,
    /// Bytecode verifier rejected a module before execution.
    VerificationFailed,
    /// A binary blob (module, script, value) was malformed.
...
impl fmt::Display for ExecutionErrorKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ExecutionErrorKind::VerificationFailed => write!(f, "VerificationFailed"),
            ExecutionErrorKind::DeserializationFailed => write!(f, "DeserializationFailed"),
            ExecutionErrorKind::LinkError => write!(f, "LinkError"),
...
pub enum ExecutionResult {
    ...
    ///
    /// `code` follows the `errors.move` convention (upper byte
    /// categorises, lower bytes carry a module-specific reason).
    /// `message` is populated when the abort uses the message form;
    /// `None` for code-only aborts.
